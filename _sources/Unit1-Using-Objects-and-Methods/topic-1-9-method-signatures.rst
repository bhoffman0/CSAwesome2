...
Multiple methods are used to organize the code by function and reduce its complexity and to avoid the repetition of code. For example, the song "Old MacDonald Had a Farm" has a lot of repetition. Instead of writing the same code over and over in order to print the song, we can write methods that contain the repeated code and call them when needed. Click on all the repeated code in the song below and then we will learn how it can be replaced with methods. 
...
The Java |visualization| below shows how a song can be divided up into methods. Click on the next button below the code to step through the code and watch the red arrow jump to the method that is being run.  Notice that when a method ends, it returns to the line right after the method call. Execution in Java always begins in the ``main`` method in the current class. Then, the **flow of control** skips from method to method as they are called.  The ``main()`` method below calls the ``intro()`` method which calls the ``chorus()`` method. When you call the chorus() method, it skips to the chorus code, executes and prints out the chorus "E-I-E-I-O", and then returns back to the method that called it. Just like variable names, method names should be descriptive of what the method does. The methods can be in any order but usually programmers put the main method at the end after all the methods that it uses are defined.
...
.. activecode:: song-add-calls
    ...
    Add lines of code to the main method for the second verse of the Old MacDonald Song by calling the intro() and chorus() methods and printing out lines about a duck or another animal.
    ~~~~
    public class OldMacDonaldSong
    {
      public static void intro()
      ...
      public static void main(String[] args)
      {
        ...
        // 2. Print out lines of the song about a duck or another animal and their sounds.
...
When we use methods in a library or API like, we can look up the **method signature** (or **method header**) in its documentation.  A **method signature** is the first line of a method that includes the method name, the return type, and the parameter list. The **return type** is the type of value that the method returns; in this lesson, we'll just look at **void** return types which means the method doesn't return anything. The **parameter list** is a list of variables and their data types that are passed to the method when it is called. The parameter list is enclosed in parentheses and separated by commas, and it can be empty with no parameters although the parentheses must be present. For example, the ``PrintStream`` class documented in https://docs.oracle.com/javase/8/docs/api/java/io/PrintStream.html? contains the method signature ``void println()`` which has an empty parameter list with no parameters and the method signature ``void println(String x)`` which prints out the given ``String`` parameter.

Let's take another look at the Old MacDonald Song and see if we can replace more repeated code with methods with parameters. Click on the words that are different in the lines that are repeated to discover what parameters we need to add to the methods.
...
We can make methods even more powerful and more abstract by giving them parameters for data that they need to do their job. A **parameter** is a variable declared in the header of a method or constructor and can be used inside the body of the method. This allows values or arguments to be passed and used by a method. An **argument** is a value that is passed into a method when the method is called. The arguments passed to a method must be compatible in number and order with the types identified in the parameter list of the method signature. 
...
And the main method can now just consist of calls to the intro() and verse() methods.
...
Click on the next button below to step through the code in the |visualizer2| and watch how the argument are saved in the parameter variables with each call to verse.
...
.. activecode:: OldMacDonaldSong2
  ...
  Run the following code to see the song Old MacDonald print out using the verse and chorus methods.  You can also see this code run in the |visualizer| by clicking on the Show Code Lens button below. Add another verse with the animal goose and the sound honk by calling the verse method with the appropriate arguments. Then call intro again. Repeat with another animal and sound of your choice.
  ~~~~
  public class OldMacDonaldSong
  {
      public static void intro()
...
Here's what that looks like with the two method calls above. Notice how the parameter variables get new values with every method call.
